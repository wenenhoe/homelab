...
main-domain, openbao-controller-role-id/-secret-id, and every
cloudflare-r2-*/backblaze-b2-*/oci-* entry don't, and stay in the
controller-side file cache instead — see secrets_registry.yaml's own
header comment for why. Vault-backed entries need OpenBao reachable and
the controller AppRole already provisioned (docs/openbao-auth.md's
runbook) before this script can do anything with them.
