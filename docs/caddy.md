...
The `caddy` role runs on **every** host in the inventory, giving each host its own reverse proxy fronting only the apps it locally runs. See [ADR 0002](decisions/0002-reverse-proxy-configuration-reproducible-from-repo/revision-000.md) for why Caddy over a UI-configured proxy like Nginx Proxy Manager.
...
3. A single wildcard vhost, `*.{{ caddy_domain }}`, with one `handle`
   ...
   [ADR 0003](decisions/0003-certificate-issuance-for-proxied-apps/revision-000.md) for
