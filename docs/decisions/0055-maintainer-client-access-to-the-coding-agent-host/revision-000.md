...
The workstation (VM 401 today, per [`tofu-vm-provisioning.md`](../../projects/tofu-vm-provisioning.md)) runs the repo's Tofu and Ansible tooling and holds the shared infrastructure SSH key. Under [ADR 0050](../0050-agent-authored-changes-reaching-production/revision-000.md) it also holds the only push credential. It runs Ubuntu 26.04 Desktop, whose GNOME session is Wayland-only, so each identity needs its own desktop session ([ADR 0057](../0057-managing-the-maintainer-workstation-from-the-repo/revision-000.md)).
...
- **Claim:** separation by OS account, with a desktop session per identity, is a sufficient boundary between agent-facing sessions and the credential-holding accounts.
  ...
  **Checked by:** the spike in [ADR 0057](../0057-managing-the-maintainer-workstation-from-the-repo/revision-000.md), then a review of what the accounts can reach.
