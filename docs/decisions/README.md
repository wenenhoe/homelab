...
| [0044](0044-prod-automation-trigger-and-execution/revision-000.md) | **Trigger and execution of prod-touching automation** — How deploys and rotations that touch prod are triggered and run, without GitHub dispatching a job to a prod-reaching host. | Undecided between: (original) A pull-based CD agent polling origin/main, not a GitHub-dispatched runner; or (revision 1) A private, LAN-only Gitea or Forgejo instance with Actions and a runner on the agent host | Working (original), Working (revision 1) | Related: [0020](0020-automation-identity-and-access-scope/revision-000.md), [0023](0023-reusing-cloud-credential-logic-with-the-secrets-store/revision-000.md) |
...
| [0042](0042-monitoring-that-survives-loss-of-the-homelab/revision-000.md) | **Monitoring that survives loss of the homelab** — Something outside the homelab notices when the homelab or its connectivity goes down. | Staged: bring the Tailscale subnet router under management, then a dedicated on-prem host, then a GCP e2-micro | Working | Related: [0010](0010-preventing-homelab-side-deletion-of-offsite-copies/revision-000.md) |
