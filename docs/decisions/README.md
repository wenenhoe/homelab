...
| [0016](0016-oci-expiry-via-scim-not-self-tracked-cache-files.md) | Accepted | OCI expiry moves off self-tracked cache files onto native `expiresOn` via Identity Domains SCIM, authenticated with a Confidential Application's OAuth2 client credentials instead of the classic OCID+PEM keypair. |
