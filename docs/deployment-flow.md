...
Installs Docker Engine, resolves every host's `compose_apps` against the central `app_registry` (see below), and resolves every value `secrets_registry.yaml` declares — generated by Ansible itself (`tinyauth`'s LDAP observer password, `lldap`'s JWT/key-seed/user-pass, `shlink`'s API key, SeaweedFS's S3 credentials) or supplied manually ahead of time (`main_domain`, DigitalOcean API key, Let's Encrypt email, Diun's Telegram token/chat ID, Wetty's default SSH host, Beszel's hub key/agent token) — via the `secrets` role. No interactive prompts: run `python3 ansible/bootstrap_secrets.py` once before your first deploy to fill in anything Ansible can't generate. See [`disaster-recovery.md`](disaster-recovery.md#secrets). No containers start yet.
...
| `secrets` | Generates (once) or reuses (every run after) every internally-generatable secret declared in `secrets_registry.yaml`, caching each on the controller. See [`disaster-recovery.md`](disaster-recovery.md#secrets). |
...
`group_vars/all/app_registry.yaml` defines `app_registry`: a single source of truth for everything about an app that does **not** vary per host — directories to create, named Docker volumes to provision (see [`volumes.md`](volumes.md)), scripts/config templates to render, and its Caddy upstream/auth behavior. A `configs` entry is how any templated file (including `.env` files) gets rendered; `no_log: true` is used for anything containing a real secret, so a run with `--diff` never prints it (see [`disaster-recovery.md`](disaster-recovery.md#secrets)'s "`no_log: true`" note).
