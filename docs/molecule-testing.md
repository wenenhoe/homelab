...
| `openbao_cert` | *(none)* | Not yet written. Needs a `default` scenario mirroring `lldap_cert`'s (real step-ca + real cert issuance against a throwaway OpenBao target, real renewal-unit side effects) and a `not_running` guard scenario, before Track A stage 1 can be marked proven — see `docs/openbao-migration-roadmap.md`. |
