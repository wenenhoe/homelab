...
| | `rotate_secret` | Genuinely imports and runs the real `playbooks/rotate-secret.yaml` (not a reimplementation) against the same real OpenBao target as `vault_backed`: creates a secret via the real `secrets` role, rotates it, and confirms the value actually changed and Vault's own KV version incremented by exactly one — proving an update-in-place, not a delete-and-recreate (the property this playbook's design depends on, since `controller`'s policy grants no `delete` on `hosts/*`). No `idempotence` step: rotation is supposed to change something every run. |
...
| `openbao` | *(none)* | Not yet written. Deploys openbao and guards the data-volume chown (see the role itself) — exercised indirectly today via `openbao_cert`'s own `default` scenario, which now deploys through this role rather than a reimplemented sequence, but has no scenario asserting this role's own guard logic (e.g. that a second run skips the chown) in isolation. |
| `openbao_cert` | `default` | Deploys the real openbao compose stack on cold (unseeded) `data`/`certs` volumes via `roles/openbao`, confirms the data volume was chowned to the image's own non-root `openbao` user, issues the initial cert against a real step-ca, confirms it's chowned back to `openbao` (not left root-owned from `step ca certificate`'s own `--user root` issuance), confirms openbao recovers from the resulting crash loop, and confirms the systemd renewal units — including the openbao-only, `%i`-guarded lines the shared `cert-renewer@.service` template carries (a chown, and a `SIGHUP` instead of a restart). Runs the renewal `ExecStart=`/`ExecStartPost` commands directly and asserts their real side effects: cert serial changes, ownership handed back to `openbao` again, and — via a raw `openssl s_client` handshake, not `bao status` — the TLS listener actually serving the renewed cert afterwards, without the container restarting. |
| | `not_running` | No running openbao target at all — the guard at the top of the role fails loudly, naming the missing container, before anything step-ca-related runs. |
| `openbao_backup` | `default` | Every task is a plain `file`/`template` render, no `when:`/loops, so one scenario covers all of it. Confirms real content, not just task success: rendered `rclone.conf` carries the right per-remote credentials (and not the other remote's), the rendered GPG public key is byte-identical to `ansible/files/backup-gpg-public-key.asc`, and the push script's `compose_deploy_dir` Jinja was actually substituted (asserted against the decoded file content, not just that the template task reported changed) with no raw `{{` left and no credential material baked in. |
