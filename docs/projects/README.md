...
| [`openbao-cli-standardization.md`](openbao-cli-standardization.md) | In progress | Replace the four overlapping bao CLI access patterns (alias, docker exec, docker run, per-host scripts) with a native binary on security and controller, then consolidate the docs describing it. |
