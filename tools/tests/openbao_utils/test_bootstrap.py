...
class SecretsDirTestCase(unittest.TestCase):
    ...
    bootstrap' own imported SECRETS_DIR (used directly by
    ...
    def setUp(self):
        ...
        patcher = patch.object(bootstrap, "SECRETS_DIR", self.tmp)
        patcher.start()
...
class ReadCacheFileTests(SecretsDirTestCase):
    def test_returns_none_when_missing(self):
        self.assertIsNone(bootstrap.read_cache_file("does-not-exist"))

    def test_returns_content_when_present(self):
        self.seed("main-domain", "example.com")
        self.assertEqual(bootstrap.read_cache_file("main-domain"), "example.com")
...
class MainVaultEntriesTests(SecretsDirTestCase):
    def setUp(self):
        ...
        fetch_patcher = patch.object(bootstrap, "fetch_root_cert", return_value="fake-root-cert")
        fetch_patcher.start()
        self.addCleanup(fetch_patcher.stop)

        login_patcher = patch.object(bootstrap, "vault_login")
        login_patcher.start()
    ...
    def test_creates_a_missing_vault_entry(self, mock_prompt, mock_read, mock_write):
        ...
        self.assertIs(bootstrap.vault_login.call_args.args[0], client_arg)
